...
public class StrategyMoveHorizontalEsquerda implements StrategyMoveInterface {
	private ELimiteCasas limitacaoCasas;

	public StrategyMoveHorizontalEsquerda() {}
	...
	public List<String> movePool(Tabuleiro tabuleiro, Peca peca) {
		...
		int x = 1;
		boolean parar = false;
		while ((x <= this.limitacaoCasas.getLimiteCasas()) && (!parar) && (colunaAuxiliar - x >= 0)) {
